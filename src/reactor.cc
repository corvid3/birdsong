...
struct Reactor::Data
{
  ...
  unsigned allocate_pfd()
  {
    ...
    pollfds.resize(pollfds.size() * 1.5f, pollfd{ -1, 0, 0 });
    wakers.resize(wakers.size() * 1.5f);
    return prev_size;
  }
...
Reactor::poll(bool)
{
  ...
  if (::poll(trans->pollfds.data(), trans->pollfds.size(), 10) == -1)
    throw std::runtime_error("fatal poll error in reactor!");
  trans->laps++;
